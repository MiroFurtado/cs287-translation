...
# For visualization of the beam
def display_beam(stack, EN_vocab, show_token=False):
  # If show_token then each node is the index of the word. Otherwise, it's the actual word
...
def beam_decode(encoded_summary, decoder_model, maxlen, beam_width, device, encoded_context=None, num_hypotheses_out=None):
    ...
    for t in range(maxlen):
        ...
        K = beam_width
        if t > 0:
          tscores += kscores # cumulative log prob
          if t == maxlen - 1:
            K = num_hypotheses_out
        
        kscores, kwords, kprev_idxs, kstates = get_top_k(K, tscores, state, disallowed_preds=[0, 1, 2])
        ...
        # Check for eos. If appears at top of beam, returns True. If eos appears otherwise, then records appearance in early_eos and 
        eos_result = check_eos(kwords, kscores, early_eos, t)
        if eos_result:
          # If ready to quit early, choose the top num_hypotheses_out, not the top beam_width hypotheses
          kscores, kwords, kprev_idxs, kstates = get_top_k(num_hypotheses_out, tscores, state, disallowed_preds=[0, 1, 2])
          bm = Beam(
              beam_words     = kwords.clone(),
              beam_prev_idxs = kprev_idxs.clone(),
              beam_scores    = kscores.clone(),
...
def parse_arguments():
    ...
    p.add_argument('vocab', type=open_rb,
                   help='File of DE vocab and EN vocab as NamedFields')
...
def main():
    ...
    for i, sentence in tqdm(enumerate(open("source_test.txt")), total=800, position=0):
        ...
        words, _, avgscores, stack = beam_decode(encoded_summary, decoder, maxlen=args.maxlen, beam_width=args.beam_width, device=device, encoded_context=encoded_context, num_hypotheses_out=args.hypotheses)
        ...
        if args.writebeam:
            ...
            plt.savefig(args.prefix + "_beam.png")
    if args.writepreds:
        f.close()
